					...
					foreach ($advisors as $key => $value) { 
						...
						$project = $this->common_model->get_project_name($value['project']); ?>
					 	<tr>
					 		<td><?php echo $i; ?></td>
					 		<td><?php echo $name; ?></td>
					 		<td><a href="<?php echo base_url().'view_callbacks?report=site_visit&advisor='.urlencode($key).'&dept='.urlencode($dept).'&city='.urlencode($city).'&fromDate='.urlencode($fromDate).'&toDate='.urlencode($toDate).'&status='.($facetoface?'9':'6'); ?>"><?php echo $value['count']; ?></a></td>
